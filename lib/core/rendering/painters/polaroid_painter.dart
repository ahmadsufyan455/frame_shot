...
class PolaroidPainter extends FramePainter {
  ...
  double get _cardPadding => imageSize.width * frameWeightMultiplier;
  double get _cardBottomPadding =>
      imageSize.width * (frameWeightMultiplier + 0.09);

  @override
  Size calculateTotalSize(Size imageSize) {
    final outerPad = imageSize.width * (frameWeightMultiplier + 0.05);
    return Size(
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final cardPhotoHeight =
        (imageSize.height / imageSize.width) * (cardWidth - _cardPadding * 2);
    final cardHeight = cardPhotoHeight + _cardPadding + _cardBottomPadding;
  ...
  void _paintCaptionOnCard(Canvas canvas, Rect cardRect, Rect photoRect) {
    ...
    tp.paint(
      canvas,
      Offset(cardRect.left + (cardRect.width - tp.width) / 2, captionY),
    );
  }
